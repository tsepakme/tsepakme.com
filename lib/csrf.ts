...
class CSRFTokenManager {
  private tokens: Map<string, { timestamp: number; sessionId?: string }> =
    new Map();

  /**
   * Generate a new CSRF token
   * @param sessionId Optional session identifier to associate with the token
  ...
  async validate(token?: string | null, sessionId?: string): Promise<boolean> {
    ...
    const isValid =
      Date.now() - data.timestamp < TOKEN_LIFETIME * 1000 &&
